...
export function MediaRow({ title, items, className, autoPlay = false, mediaType }: MediaRowProps) {
  const plugin = useRef(
      ...
      <div className="flex items-center justify-between px-4 md:px-8">
        {mediaType ? (
          <Link 
            ...
            params={{ mediaType }}
            search={{ page: 1 }}
            className="text-xl md:text-2xl font-bold text-slate-200 hover:text-white transition-colors cursor-pointer flex items-center gap-2 group"
          >
            {title}
        ...
        {mediaType && (
          <Button variant="ghost" className="text-sm font-semibold text-slate-400 hover:text-white hover:bg-transparent group cursor-pointer" asChild>
            <Link 
              to="/browse/$mediaType" 
              params={{ mediaType }}
              search={{ page: 1 }}
